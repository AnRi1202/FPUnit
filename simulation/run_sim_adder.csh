...
# -64bit      : 64-bit mode
# -f          : ファイルリスト (compile_adder.f)
# -top        : トップレベルエンティティ名 (VHDL)
# -access     : 波形などのアクセス権
# -timescale  : timescale が書いてないモジュールに 1ns/1ps をデフォルト適用
# Note: xrunのデフォルトはVHDL-2008なので、to_hstring関数が使用可能
xrun -v200x -64bit -f compile_adder.f -top tb_fpadd_kintex -access +rwc -timescale 1ns/1ps |& tee xrun_adder.log

echo "Xcelium (xrun) simulation (compile + elaborate + run) finished. Log: xrun_adder.log"

...
from sweep_paths import CATEGORY_NO_RETIME, syn_dir_from_here, find_latest_run
...
file mkdir sweep_run/no_retime
...
def make_tcl_v1_area_opt():
    return f"""
{common_header()}
set run_dir [file normalize "sweep_run/no_retime/run-v1_area_opt_ret-P1-noRetime-$tag"]
...
def make_tcl_v1_3_addmul():
    return f"""
{common_header()}
set run_dir [file normalize "sweep_run/no_retime/run-v1_3_addmul_ret-P1-noRetime-$tag"]
set WORK_DIR [file normalize "${{run_dir}}/WORK"]
file mkdir $run_dir
file mkdir $WORK_DIR
define_design_lib WORK -path $WORK_DIR
...
def make_tcl_v1_4_divsqrt():
    return f"""
{common_header()}
set run_dir [file normalize "sweep_run/no_retime/run-v1_4_divsqrt_ret-P1-noRetime-$tag"]
set WORK_DIR [file normalize "${{run_dir}}/WORK"]
file mkdir $run_dir
file mkdir $WORK_DIR
define_design_lib WORK -path $WORK_DIR
...
def make_tcl_v2_fpall():
    return f"""
{common_header()}
set run_dir [file normalize "sweep_run/no_retime/run-v2_fpall_ret-P1-noRetime-$tag"]
set WORK_DIR [file normalize "${{run_dir}}/WORK"]
file mkdir $run_dir
file mkdir $WORK_DIR
define_design_lib WORK -path $WORK_DIR
...
def make_tcl_v2_1_add():
    return f"""
{common_header()}
set run_dir [file normalize "sweep_run/no_retime/run-bf16_add_ret-P1-noRetime-$tag"]
set WORK_DIR [file normalize "${{run_dir}}/WORK"]
file mkdir $run_dir
file mkdir $WORK_DIR
define_design_lib WORK -path $WORK_DIR
...
def make_tcl_v2_2_mult():
    return f"""
{common_header()}
set run_dir [file normalize "sweep_run/no_retime/run-bf16_mult_ret-P1-noRetime-$tag"]
set WORK_DIR [file normalize "${{run_dir}}/WORK"]
file mkdir $run_dir
file mkdir $WORK_DIR
define_design_lib WORK -path $WORK_DIR
...
def make_tcl_v2_3_addmul():
    return f"""
{common_header()}
set run_dir [file normalize "sweep_run/no_retime/run-v2_3_addmul_ret-P1-noRetime-$tag"]
set WORK_DIR [file normalize "${{run_dir}}/WORK"]
file mkdir $run_dir
file mkdir $WORK_DIR
define_design_lib WORK -path $WORK_DIR
...
def make_tcl_flopoco_origin_fpadd():
    return f"""
{common_header()}
set run_dir [file normalize "sweep_run/no_retime/run-flopoco_origin_fpadd_ret-P1-noRetime-$tag"]
set WORK_DIR [file normalize "${{run_dir}}/WORK"]
file mkdir $run_dir
file mkdir $WORK_DIR
define_design_lib WORK -path $WORK_DIR
...
def make_tcl_flopoco_origin_fpmult():
    return f"""
{common_header()}
set run_dir [file normalize "sweep_run/no_retime/run-flopoco_origin_fpmult_ret-P1-noRetime-$tag"]
set WORK_DIR [file normalize "${{run_dir}}/WORK"]
file mkdir $run_dir
file mkdir $WORK_DIR
define_design_lib WORK -path $WORK_DIR
...
def make_tcl_flopoco_origin_addmul():
    return f"""
{common_header()}
set run_dir [file normalize "sweep_run/no_retime/run-flopoco_origin_addmul_ret-P1-noRetime-$tag"]
set WORK_DIR [file normalize "${{run_dir}}/WORK"]
file mkdir $run_dir
file mkdir $WORK_DIR
define_design_lib WORK -path $WORK_DIR
...
def make_tcl_flopoco_origin_addmul_bf16():
    return f"""
{common_header()}
set run_dir [file normalize "sweep_run/no_retime/run-flopoco_origin_addmul_bf16_ret-P1-noRetime-$tag"]
set WORK_DIR [file normalize "${{run_dir}}/WORK"]
file mkdir $run_dir
file mkdir $WORK_DIR
define_design_lib WORK -path $WORK_DIR
...
def make_tcl_flopoco_origin_divsqrt():
    return f"""
{common_header()}
set run_dir [file normalize "sweep_run/no_retime/run-flopoco_origin_divsqrt_ret-P1-noRetime-$tag"]
set WORK_DIR [file normalize "${{run_dir}}/WORK"]
file mkdir $run_dir
file mkdir $WORK_DIR
define_design_lib WORK -path $WORK_DIR
...
def make_tcl_v2_4_divsqrt():
    return f"""
{common_header()}
set run_dir [file normalize "sweep_run/no_retime/run-v2_4_divsqrt_ret-P1-noRetime-$tag"]
set WORK_DIR [file normalize "${{run_dir}}/WORK"]
file mkdir $run_dir
file mkdir $WORK_DIR
define_design_lib WORK -path $WORK_DIR
...
def main():
    ...
    for label, make_tcl_fn, run_pattern in designs_to_run:
        ...
        try:
            ...
            latest = find_latest_run(syn_dir, run_pattern, CATEGORY_NO_RETIME)
            if latest:

...
from sweep_paths import CATEGORY_RETIME, tcl_path as tcl_relpath, syn_dir_from_here, join_run_dir
...
set run_dir  "$sweep_root/run-fpmult_with_bf16_f{freq}"
...
for freq in frequencies:
    ...
    tcl_out = os.path.join(os.path.dirname(__file__), f"run_fpmult_with_bf16_f{freq}.tcl")
    tcl_rel = os.path.relpath(tcl_out, syn_dir)
    with open(tcl_out, "w") as f:
        f.write(tcl_content)
    print(f"f{freq}: wrote {tcl_out}")

    # 3. Run synthesis
    run_dir = join_run_dir(syn_dir, f"run-fpmult_with_bf16_f{freq}", CATEGORY_RETIME)
    log_path = os.path.join(syn_dir, "logs", f"run_fpmult_with_bf16_f{freq}.log")
    os.makedirs(os.path.join(syn_dir, "logs"), exist_ok=True)
    cmd = f"source ~/.cshrc; cd {syn_dir} && dc_shell-xg-t -f {tcl_rel} >& {log_path}"

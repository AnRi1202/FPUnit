...
from sweep_paths import CATEGORY_RETIME, tcl_path, syn_dir_from_here, find_latest_run_pattern
...
scripts = {
    "addmul": {
        "tcl": "ret_flopoco_origin_addmul.tcl",
        "label": "flopoco_origin_addmul_ret",
        "csv": "sweep_summary_flopoco_origin_addmul.csv"
    }
}
...
for key, cfg in scripts.items():
    ...
    for pipe in pipeline_stages:
        ...
        full_cmd = f"source ~/.cshrc; setenv NUM_PIPE {pipe}; dc_shell-xg-t -f {tcl_path(cfg['tcl'])} >& {log_file}"

        try:
            subprocess.run(["tcsh", "-c", full_cmd], check=True)

            # Find the run directory (it has a timestamp tag)
            latest_run = find_latest_run_pattern(syn_dir, f"run-{cfg['label']}-P{pipe}-T{clock_period}-.*", CATEGORY_RETIME)
            if latest_run:

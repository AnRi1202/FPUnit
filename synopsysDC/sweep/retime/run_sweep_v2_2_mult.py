...
scripts = {
    "bf16_mult_ret": {
        "tcl": "ret_v2_2_mult.tcl",
        "label": "bf16_mult_ret",
        "csv": "sweep_summary_v2_2_mult.csv"
    }
}
...
for key, cfg in scripts.items():
    ...
    for pipe in pipeline_stages:
        ...
        # ret_v2_2_mult.tcl uses env(PIPE)
        full_cmd = f"source ~/.cshrc; setenv PIPE {pipe}; dc_shell-xg-t -f {tcl_path(cfg['tcl'])} >& {log_file}"
        ...
        try:
            ...
            latest_run = find_latest_run_pattern(syn_dir, pattern, CATEGORY_RETIME)
            if latest_run:

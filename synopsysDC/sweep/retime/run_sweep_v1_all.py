...
from sweep_paths import CATEGORY_RETIME, tcl_path, syn_dir_from_here, join_run_dir
...
v1_dir = f"{base_rtl_dir}/v1_area_opt"
v2_dir = f"{base_rtl_dir}/v2_bf16_full" # for utils.vhdl
...
elaborate area_opt_ret -library WORK -parameters "PARAM_PIPE_AM=${{num_pipe}},PARAM_PIPE_DS=${{num_pipe}}"
...
for pipe in pipeline_stages:
    ...
    run_dir = join_run_dir(syn_dir, run_dir_name, CATEGORY_RETIME)
    ...
    full_cmd = f"source ~/.cshrc; dc_shell-xg-t -f {tcl_path(os.path.basename(tcl_file)) if not os.path.isabs(tcl_file) else os.path.relpath(tcl_file, syn_dir)} >& {log_file}"
...
# Save CSV
with open("sweep_summary_v1_all.csv", "w") as f:
    f.write("PipelineStages,Area,MaxDataArrival\n")
    for r in results:
        f.write(f"{r[0]},{r[1]},{r[2]}\n")

print("v1_area_opt Sweep Completed.")

#!/usr/bin/env python3
"""
Pipeline sweep for FPMult_origin_ret (FloPoCo FP32 Mult only).
...
# Configuration
clock_period = 0.5
# Use command line to limit stages: python run_sweep_flopoco_origin_fpmult.py 1 2 3
pipeline_stages = [int(a) for a in sys.argv[1:]] if len(sys.argv) > 1 else list(range(1, 19))

label = "flopoco_origin_fpmult_ret"
tcl_file = "ret_flopoco_origin_fpmult.tcl"
csv_file = "sweep_summary_flopoco_origin_fpmult.csv"

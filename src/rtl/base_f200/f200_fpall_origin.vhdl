...
begin
    ...
    GEN_ADD: if NUM_OPS /= 3 and NUM_OPS /= 7 generate
        U_ADD: FPAdd_8_23_Freq200_uid2 port map(clk=>clk, X=>fpc_X, Y=>fpc_Y, R=>add_R);
    end generate;
    ...
    GEN_MUL: if NUM_OPS /= 1 and NUM_OPS /= 5 generate
        U_MUL: FPMult_8_23_uid2_Freq200_uid3 port map(clk=>clk, X=>fpc_X, Y=>fpc_Y, R=>mul_R);
    end generate;
    ...
    GEN_SQRT_DIV: if NUM_OPS = 4 or NUM_OPS = 6 generate
        ...
        U_DIV:  FPDiv_8_23_Freq200_uid2 port map(clk=>clk, X=>fpc_X, Y=>fpc_Y, R=>div_R);
    end generate;
    ...
    GEN_BF16: if NUM_OPS >= 5 generate
        ...
        GEN_BF_ADD: if NUM_OPS = 5 or NUM_OPS = 6 generate
            -- Upper lane [33:16]
            U_BFADD_H: FPAdd_8_7_Freq200_uid2 port map(clk=>clk, X=>fpc_X_upper, Y=>fpc_Y_upper, R=>bfadd_h);
            -- Lower lane [15:0]
        ...
        GEN_BF_MUL: if NUM_OPS = 6 or NUM_OPS = 7 generate
            -- Upper lane [33:16]
            U_BFMUL_H: FPMult_8_7_uid2_Freq200_uid3 port map(clk=>clk, X=>fpc_X_upper, Y=>fpc_Y_upper, R=>bfmul_h);
            -- Lower lane [15:0]
